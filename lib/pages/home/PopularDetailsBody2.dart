...
class AverageDetailsBody extends StatefulWidget {
  final String averageDhobiId;

  const AverageDetailsBody({Key? key, required this.averageDhobiId})
      : super(key: key);

  @override
  _AverageDetailsBodyState createState() => _AverageDetailsBodyState();
}
class _AverageDetailsBodyState extends State<AverageDetailsBody> {
  ...
  Future<void> fetchDataFromCollection() async {
    ...
    try {
      DocumentSnapshot averageDhobiDoc = await _firestore
          .collection('dhobi')
          .doc('FwLIVilNtChUZzLbceDS')
          .collection('Average dhobi')
          .doc(widget.averageDhobiId)
          .get();

      if (averageDhobiDoc.exists) {
        var data = averageDhobiDoc.data() as Map<String, dynamic>;
        ...
        print("No document found for this average dhobi.");
      }
    } catch (e) {
              ...
              child: Column(
                ...
                children: [
                  const HorizontalTextLine(
                      text: "Wash & Iron", color: Colors.grey),
