...
import 'package:wash_it/pages/home/popular_details_body.dart';
import '../../no_internet.dart';
import '../../widgets/shimmer.dart';
import '../Cart/cart.dart';
import '../Cart/cart_provider.dart';

class PremiumDhobiPage extends StatefulWidget {
  @override
  _PremiumDhobiPageState createState() => _PremiumDhobiPageState();
}

class _PremiumDhobiPageState extends State<PremiumDhobiPage> {
  ...
  Future<void> _fetchDhobis() async {
    ...
    try {
      QuerySnapshot dhobiSnapshot = await FirebaseFirestore.instance.collection('dhobi').limit(1).get();
      List<DocumentSnapshot> premiumDhobiList = [];

      for (var dhobiDoc in dhobiSnapshot.docs) {
        Query query = dhobiDoc.reference.collection('Premium dhobi').orderBy('name').limit(_pageSize);

        if (_lastDocument != null) {
          query = query.startAfterDocument(_lastDocument!);
        }

        QuerySnapshot premiumSnapshot = await query.get();
      ...
      setState(() {
        _dhobiDocs.addAll(premiumDhobiList);
        _hasMore = premiumDhobiList.length == _pageSize;
      });
    } catch (e) {
      print('Error fetching Laundries: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error fetching Laundries. Please try again later.')),
      );
    } finally {
      setState(() {
        _isLoading = false;
  ...
  AppBar _buildAppBar() {
    return AppBar(
      ...
      actions: [
        Consumer<CombinedDhobiCartProvider>(builder: (context, cartProvider, child) {
  ...
  Widget _buildBody() {
    ...
    if (_dhobiDocs.isEmpty) {
      return const Center(child: Text('No Laundries found. Please check back later.'));
    }

    return ListView.builder(
      controller: _scrollController,
      padding: const EdgeInsets.all(15.0),
      itemCount: _dhobiDocs.length + (_hasMore ? 1 : 0) + (!_hasMore && !_isLoading ? 1 : 0),
      itemBuilder: (context, index) {
        if (index < _dhobiDocs.length) {
          return _buildPremiumDhobi(_dhobiDocs[index]);
        } else if (_hasMore) {
          ...
          children: [
            ...
            Image.network(
              ...
              errorBuilder: (context, error, stackTrace) {
                return const Icon(Icons.error, size: 100, color: Colors.red); // Fallback if the GIF fails to load
              },
            ),
            const SizedBox(height: 10), // Add some space between the GIF and the text
            const Text(
  ...
  Widget _buildPremiumDhobi(DocumentSnapshot dhobiDoc) {
    final data = dhobiDoc.data() as Map<String, dynamic>?;

    if (data == null) {
      return const Center(child: Text('No Premium Laundries found.'));
    }
  ...
  void _onDhobiTap(String dhobiId) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => PopularDetailsBody(premiumDhobiId: dhobiId,)),
    );
  }
  Widget _buildCarousel(List<String> images) {
    ...
    return CarouselSlider(
      ...
      items: images.map((imageUrl) {
        return Container(
          width: 400, // Set the width for the images
          margin: const EdgeInsets.symmetric(horizontal: 5.0), // Set horizontal margins
          child: ClipRRect(
            borderRadius: BorderRadius.circular(15),
            child: Image.network(
              imageUrl,
              fit: BoxFit.cover, // or BoxFit.contain as per your requirement
              errorBuilder: (context, error, stackTrace) => const Icon(Icons.error, size: 100, color: Colors.red),
              loadingBuilder: (context, child, loadingProgress) =>
              loadingProgress == null ? child : const Center(child: ShimmerLoading()),
            ),
          ),
        );
      }).toList(),
  ...
  Widget _buildDhobiInfo(String name, String description, double rating, List<String> services) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        Wrap(
          ...
          children: services.map((service) {
            return Chip(label: Text(service), backgroundColor: Colors.green[100]);
          }).toList(),
        ),
      ],
    );
